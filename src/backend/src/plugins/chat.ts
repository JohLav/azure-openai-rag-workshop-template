...
export class ChatService {
  ...
  async run(messages: Message[]): Promise<ChatResponse> {
    ...
    // The object searchResults.results containing the search results is an AsyncIterator, 
    // so we need to use a for await loop to iterate over the results.
    for await (const result of searchResults.results) {
      const document = result.document;
      const sourcePage = document[this.sourcePageField];
      const content = document[this.contentField].replaceAll(/[\n\r]+/g, ' ') // Regex: replace all the new lines with spaces
      results.push(`${sourcePage}: ${content}`);
    }

    // Join all the results into a single string, and separate each document with a new line.
    // We'll use this content to generate the augmented prompt.
